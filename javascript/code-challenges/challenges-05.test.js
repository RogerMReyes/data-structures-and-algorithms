...
const countNumberOfElements = (arr) => {
  let total = arr.reduce((accumulator) => {
...
const reversedString = (str) => {
  ...
  let reversedStr = charArr.reduce((acc, value) => {
    return value + acc;
  }, '');
  return reversedStr;
};
...
const countNumberOfChildren = (arr) => {
  ...
  let total = arr.reduce((acc, value) =>{
    for(let child in value.children){
      newArr.push(value.children[child]);
      acc = acc + 1;
    }
    return acc;
  },0);
...
const calculateAverage = (arr) => {
  let averageVal = arr.reduce((acc, value) => {
...
const countPrimeNumbers = (arr) => {
  let primeNums = arr.reduce((acc, value) => {
    if(isPrime(value)){
      acc = acc + 1;
    }
    return acc;
...
const extractStat = (statName, arr) => {
  let pokemonStat = arr.reduce((acc, val) => {
    if(val.stat.name === statName){
      acc.stat.name = val.stat.name;
      acc.stat.url = val.stat.url;
      acc.effort = val.effort;
      acc.baseStat = val.baseStat;
    }
    return acc;
  },{stat:{url: '', name: ''}, effort:0, baseStat:0});
  return pokemonStat;
};
...
const extractChildren = (arr) => {
  ...
  return newArr.reduce((acc, val) => {
    for(let child in val.children){
      acc.push(val.children[child]);
    }
    return acc;
  },[]);
};
