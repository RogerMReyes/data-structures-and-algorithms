...
const replaceZeros = (string) => {
  ...
  for(let char of string){
    char.match(regex) ? newStr +='zero' : newStr += char;
  }
  return newStr;
};
...
const validateEmail = (email) => {
  let regex = /^[a-zA-Z0-9]+.[a-zA-Z0-9]+@[a-zA-Z0-9.-]+\.[a-zA-Z]{3}$|^[a-zA-Z0-9]{2,}@[a-zA-Z0-9]+\.[a-zA-Z]{3}$/mg;
  return String(email).match(regex)? true : false;
};
...
const validatePhoneNumber = (phoneNumber) => {
  let regex = /^\([0-9]+\)(\s|-)[0-9]+(\s|-)[0-9]+$|^\([0-9]+\)[0-9]+(\s|-)[0-9]+$|^[0-9]{3}(\s|-)[0-9]{3}(\s|-)[0-9]{4}$|^[0-9]{3}(\s|-)[0-9]{7}|^[0-9]{6}(\s|-)[0-9]{4}|^[0-9]{10}$/gm;
  return String(phoneNumber).match(regex)? true : false;
